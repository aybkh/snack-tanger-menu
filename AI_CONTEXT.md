# 🌮 Snack Tanger 303 - TPV System

Bienvenido a la documentación oficial del sistema **Snack Tanger 303 TPV**. 
...
Asegúrate de que cuentas con `tenant_config.json` y `snack_tanger_menu/menu_template.json` en la raíz de despliegue, junto con la carpeta de imágenes `wwwroot/products`.
...
xcopy /E /Y dist\* ..\SnackTPV.Backend\wwwroot\

# Publicar el Backend
cd ..\SnackTPV.Backend
dotnet publish -c Release -r win-x64 --self-contained -p:PublishSingleFile=true -p:PublishTrimmed=false
```
**Resultado:** Obtendrás un archivo `SnackTPV.Backend.exe` en la carpeta `publish/` que, al clicarlo, enciende base de datos, API, Sistema de Impresión y Sirve React Todo-En-Uno.
