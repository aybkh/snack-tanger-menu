...
const defaultReviews = [
    ...
    { name: "Suli Harrak", text: "Snack 303 es un sitio increíble. Comida espectacular y precios razonables. ¡Querrás volver!", stars: 5 },
...
const ReviewsSection = ({ reviews: customReviews }) => {
    const { t } = useLanguage();
    
    // Map JSON 'author' to legacy 'name' if necessary
    const activeReviews = customReviews?.length > 0 
        ? customReviews.map(r => ({ name: r.author || r.name, text: r.text, stars: r.stars || 5 }))
        : defaultReviews;
        ...
        <section className="reviews-section relative">
            <div className="section-head">
                ...
                <div style={{ fontSize: '1rem', color: '#ccc', marginTop: '10px', fontFamily: "'Montserrat', sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                    <GoogleIcon /> 4.6⭐ {t('reviews_stats')}
                </div>
            </div>

            <div className="marquee-container">
                {[...activeReviews, ...activeReviews].map((review, idx) => (
